...
import { ArrowLeft, Plus, CalendarDays, MapPin, Clock } from 'lucide-react';
...
export function TeamDetailPage({ teamId }: { teamId: string }) {
  ...
  const loadTeam = useCallback(async () => {
    const [{ data: teamData }, { data: sessionsData }] = await Promise.all([
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-ink-400">{team.sport} · {team.season} · {team.athlete_count} athletes</p>
            </div>
          </div>
        </div>
